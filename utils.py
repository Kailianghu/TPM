...
def label_encoding(tree_num_intervals, cmp_ratio, begins, ends, name="label_encoding"):
    label_dict = {}
    weight_dict = {}
    height = int(math.log2(tree_num_intervals))  # 满二叉树
    with tf.name_scope(name):
        for i in range(height):  # 树的第几层
            for j in range(2 ** i):  # 每层树的节点数
                temp_ind = max(int(tree_num_intervals * 1.0 / (2 ** i) * j) - 1, 0)
                if j == 0:
                    weight_temp = tf.where(tf.less(cmp_ratio, tf.reshape(begins[:, temp_ind], [-1, 1])),
                                           tf.zeros_like(cmp_ratio), tf.ones_like(cmp_ratio))
                else:
                    weight_temp = tf.where(tf.less(cmp_ratio, tf.reshape(ends[:, temp_ind], [-1, 1])),
                                           tf.zeros_like(cmp_ratio), tf.ones_like(cmp_ratio))
                temp_ind = max(int(tree_num_intervals * 1.0 / (2 ** i) * (j + 1)) - 1, 0)
                weight_temp = tf.where(tf.less(cmp_ratio, tf.reshape(ends[:, temp_ind], [-1, 1])), weight_temp,
                                       tf.zeros_like(cmp_ratio))
                temp_ind = max(int(tree_num_intervals * (1.0 / (2 ** i) * j + 1.0 / (2 ** (i + 1)))) - 1, 0)
                label_temp = tf.where(tf.less(cmp_ratio, tf.reshape(ends[:, temp_ind], [-1, 1])),
                                      tf.zeros_like(cmp_ratio), tf.ones_like(cmp_ratio))
...
def get_encoded_playtime(label_encoding_predict, tree_num_intervals, begins, ends, name="encoded_playtime"):
    height = int(math.log2(tree_num_intervals)) # 根节点到叶子节点中间的分类器个数。tips:整个树的下标从0开始。
    encoded_prob_list = []
    temp_encoded_playtime = (begins + ends) / 2.0  # bsx32
    encoded_playtime = temp_encoded_playtime

    for i in range(tree_num_intervals): # 每个叶子叶子节点往上回溯树深depth步即可。
        temp = 0.0
        cur_code = 2 ** height - 1 + i # [16, 32]的叶子节点编号
        for j in range(1, 1 + height):
            classifier_branch = cur_code % 2 # 判断左分支还是右分支
            classifier_idx = int((cur_code - 1) / 2) # 转到父节点。
            # update cur_code
            cur_code = classifier_idx
            if classifier_branch == 1:
                temp += tf.log(1.0 - label_encoding_predict[:, classifier_idx] + 0.00001) # 选择右子树
            else:
                temp += tf.log(label_encoding_predict[:, classifier_idx] + 0.00001) # 选择左子树
